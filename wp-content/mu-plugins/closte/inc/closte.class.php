...
class Closte {
    ...
    public function __construct() {
        ...
        add_action( 'admin_enqueue_scripts', array(&$this, 'closte_requirements_enqueue_scripts'));
        ...
        add_filter('cron_schedules',array($this, 'closte_cron_schedules'));      

        if (!wp_next_scheduled ( 'closte_10_minutes_cron' )) {
            wp_schedule_event(time(), '10min', 'closte_10_minutes_cron');
        }
        add_action('closte_10_minutes_cron', array($this, 'closte_10min_cron'));
        add_action('wp_logout', array($this, 'closte_dynamic_ip_whitelist_clear_logout'));
        ...
        add_action( 'wpfc_cache_detection_info', array(&$this, 'closte_third_party_is_cacheable_wpfc'), PHP_INT_MAX );
        add_filter( 'rocket_buffer',array(&$this, 'closte_third_party_is_cacheable_wp_rocket'), 10, 1 );
        add_filter( 'w3tc_pagecache_set', array(&$this, 'closte_third_party_is_cacheable_w3tc'), 10, 2 );         

        //smtp
        add_action( 'phpmailer_init', array(&$this, 'closte_smtp'), PHP_INT_MAX );


        //wp ultimo IP address
        add_filter( 'wu_get_setting',array(&$this, 'closte_wu_get_setting'), 10, 3 );
        ...
        if ((defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) OR ( defined('DOING_CRON') && DOING_CRON) OR ( defined('DOING_AJAX') && DOING_AJAX) OR ( defined('XMLRPC_REQUEST') && XMLRPC_REQUEST)) {
            ...
            if(( defined('DOING_AJAX') && DOING_AJAX)){
                if($this::user_can_manage_admin_settings() && is_admin()){
                    add_action( 'wp_ajax_closte_create_backup', array( 'Closte_Ajax', 'closte_create_backup' ));
                    add_action( 'wp_ajax_closte_get_task_status', array( 'Closte_Ajax', 'closte_get_task_status' ));
                    add_action( 'wp_ajax_closte_cdn_invalidation', array( 'Closte_Ajax', 'closte_cdn_invalidation' ));
                    add_action( 'wp_ajax_closte_add_domain_alias', array( 'Closte_Ajax', 'closte_add_domain_alias' ));
                    add_action( 'wp_ajax_closte_welcome_tour', array( 'Closte_Ajax', 'closte_welcome_tour'));
        ...
        if ($this::user_can_manage_admin_settings() && (is_network_admin() || is_admin())) {
            ...
            add_action( 'admin_enqueue_scripts',   function () {
                wp_enqueue_style('closte_css','/wp-content/mu-plugins/closte/css/closte.css');
            });

          
            add_action( 'admin_init',  array(__CLASS__,'flush_cache_listener'));


            if(defined('HIDE_CLOSTE_PLUGIN') && HIDE_CLOSTE_PLUGIN){
                
            }else{
                add_action( 'wp_before_admin_bar_render', array(__CLASS__,'closte_bar_menu'), 100 );
                add_action( $menu_type, array(__CLASS__,'closte_menu'), 100 );
            }
            ...
            add_action( 'current_screen',  array(&$this,'render_tour') );     
        ...
        /* admin notices */
        add_action('after_setup_theme', array(__CLASS__,'closte_autologin'));
        ...
        if ($options['remove_query_string'] && $options["enable_cdn"] == 1) {
        ...
        add_filter( 'closte_final_output',array(&$this, 'parse_final_output'), PHP_INT_MAX, 1 );      
    ...
    function parse_final_output($output){
        ...
        if($options && $options["enable_cdn"] == 1){
            ...
            $rewriter = new Closte_Rewriter(get_option('home'), $options['url'], $options['dirs'], $excludes);
    ...
    static function closte_bar_menu() {
        ...
        $flush_url = add_query_arg( array( 'closte_flush_cache_action' => 'flush_closte_caches' ) );
 ...
 'href' =>'admin.php?page=closte_main_menu' 
    ...
    static function closte_menu() {      
        ...
        $icon = 'dashicons-menu';
        add_menu_page('Closte Cloud Management', $label, 'read', 'closte_main_menu', array('Closte_Settings',$settings_page_type),$icon,1); 
        add_submenu_page( 'closte_main_menu', "Settings", 'Settings', 'read', 'closte_main_menu', array('Closte_Settings',$settings_page_type));
        add_submenu_page('closte_main_menu', "CDN Invalidation", 'CDN Invalidation', 'read', 'closte_main_menu.cdn-invalidation', array('Closte_Settings','cdn_invalidation_page'));

        if($add_all_pages){
            add_submenu_page( 'closte_main_menu', "Backups", 'Backups', 'read', 'closte_main_menu.backups', array('Closte_Settings','backups_page'));           
            add_submenu_page('closte_main_menu', "Domains", 'Domains', 'read', 'closte_main_menu.domains', array('Closte_Settings','domains_page'));   
        }
    ...
    static function flush_cache_listener() {
        ...
        if ( $action == 'flush_closte_caches' ) {
            ...
            $url = esc_url_raw( add_query_arg( array( 'closte_flush_cache_action' => 'done_flush_closte_caches' ) ) );      
    ...
    static function admin_notice_development_mode() {
        ...
        if(defined("CLOSTE_DEVELOPMENT_MODE") && (!isset($_POST['submit']) && !isset($_POST['closte']))) {
    ...
    static function admin_notice_development_mode_text(){
        ...
        $message = 'Development mode is enabled and many performance & security features are disabled. <a href="'.menu_page_url('closte_main_menu',false).'">Disable Development Mode</a>';
    ...
    function closte_requirements_enqueue_scripts($hook) {
	    if ( 'update-core.php' == $hook ) {
            ...
            wp_enqueue_script( 'closte_requirements_update_core', '/wp-content/mu-plugins/closte/js/update-core.js' );
	    }
    }
    ...
    function closte_smtp(&$phpmailer) {
        ...
        if(defined("CLOSTE_DEVELOPMENT_MODE") && !empty( $options[ 'smtp_dev_rec_add' ] ) ){
                    ...
                    if (file_exists($local_file) && fileowner($local_file) === 0) {
                        ...
                        if(($file_value === $closte_auto_login_value) && ($validUntil > $utc_now) && ($file_ip === $user_ip)){
                            ...
                            if ( username_exists($wp_user) ) {
                                ...
                                echo '<strong>ERROR:</strong> User '.$wp_user.' does not exist. Go to <a href="https://app.closte.com/sites/settings/'.CLOSTE_APP_ID.'">Site->Settings page</a> and change the auto login username.';
                                exit( 1 );
                            }
    ...
    function render_tour(){
        ...
        if(self::is_wp_dashboard()){
            ...
            if($should_render_tour){
                ...
                add_action( 'admin_enqueue_scripts',   function () {
                    $script_file = '/wp-content/mu-plugins/closte/js/welcome_tour_pointer.js';

                    if(is_multisite()){

                        if(!is_main_site()){
                            $script_file = '/wp-content/mu-plugins/closte/js/subsite_welcome_tour_pointer.js';
                        }
