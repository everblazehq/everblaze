...
class Closte_Settings
{
    ...
    public static function update(){
        ...
        if ( isset($_POST['submit']) && isset($_POST['closte']) ) {
            ...
            if(!is_network_admin()){
                ...
                if(isset($_POST['closte']['dirs'])){
                    $currentOptions['dirs'] = $_POST['closte']['dirs'];
                }else{
                    $currentOptions['dirs'] = '';
                }
                ...
                if(isset($_POST['closte']['remove_query_string'])){
                    $currentOptions['remove_query_string'] = (int)$_POST['closte']['remove_query_string'];
                }else{
                    $currentOptions['remove_query_string'] = 0;
                }
    ...
    static function admin_notice_development_mode_turned_off_text(){
        ...
        $message = 'Development mode is disabled. All caches are flushed except Google CDN. If you made changes in files (e.g CSS or JS file) that were previously cached on Google CDN network, you need to flush the CDN. <a href="'.menu_page_url('closte_main_menu.cdn-invalidation',false) .'">Clear CDN Cache</a>';
                            ...
                            <td>
                                <fieldset>
                                    <label for="CLOSTE_DIRs">
                                        <input type="text" name="closte[dirs]" id="CLOSTE_DIRs" value="<?php echo $options['dirs']; ?>" size="64" class="regular-text code" placeholder="wp-content,wp-includes,min" />
                                       </label>

                                    <p class="description">
                                        <?php _e("Assets in these directories will be pointed to the CDN URL. Enter the directories separated by", "closte"); ?>
                                        <code>,</code>
                                    </p>
                                </fieldset>
                            </td>
                            ...
                            <td>
                                <fieldset>
                                    <label for="closte_remove_query_string">
                                      ...
                                      <p class="description">Remove query string from url, eq: ?ver=1.0.0"</p>
                                </fieldset>
                            </td>
                        </tr>
