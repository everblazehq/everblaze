            ...
            if (isset($_GET['dmm_id'])) {
                ...
                <form action="<?php echo esc_url($_SERVER['REQUEST_URI']);?>" class="<?php echo esc_attr(get_option('dmm_form_cls'));?>" method="post">
                    ...
                    <?php if (isset($dmm_fields['Address']['active']) && $dmm_fields['Address']['active']) { ?>
                        <p>
                            <?php echo esc_html_e('Street', 'doneren-met-mollie') . (isset($dmm_fields['Address']['required']) && $dmm_fields['Address']['required'] ? '<span style="color:red;">*</span>' : '') . '<br>';?>
                    ...
                    <p>
                        <?php
                        echo esc_html_e('Amount', 'doneren-met-mollie') . ' ' . (get_option('dmm_currency_switch') != '1' ? dmm_get_currency_symbol(get_option('dmm_currency')) : '') . '<span style="color:red;">*</span><br>';
                        ...
                        if (get_option('dmm_free_input'))
                        {
                            ...
                            {
                                echo '<select name="dmm_currency" id="dmm_currency" onchange="dmm_multicurrency_methods(this.value);" style="width: 20%;float:left;">';

                                foreach (dmm_get_currencies() as $currency => $decimals)
                                    echo '<option ' . (get_option('dmm_currency') == $currency ? 'selected' : '') . '>' . esc_html($currency) . '</option>';

                                echo '</select>';
                            }
                            else
                                echo '<input type="hidden" name="dmm_currency" id="dmm_currency" value="' . get_option('dmm_currency', 'EUR') . '">';

                            echo '<span style="display:block;overflow:auto;"><input type="text" id="dmm_amount" name="dmm_amount" class="' . esc_attr(get_option('dmm_fields_cls')) . '" value="' . (isset($_POST["dmm_amount"]) ? esc_attr($_POST["dmm_amount"]) : get_option('dmm_default_amount')) . '" style="width: 100%;float:left;"></span>';
                        }
                        else
                            echo '<input type="hidden" name="dmm_currency" id="dmm_currency" value="' . get_option('dmm_currency', 'EUR') . '">';
                        ?>
